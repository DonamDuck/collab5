...
#### 반응형 타입 스케일 (2026-07-31 확정 — 홈 적용, 신규 화면의 기본값)

> 계단뿌셔클럽 fresh-load 실측 기반(대표 확정). **핵심 원리 = 모바일→데스크탑에서 제목만 키우고 본문은 거의 안 키운다.** 넓은 화면일수록 위계 대비가 벌어진다(모바일 1.75배 → 데스크탑 2.35배).

| 역할 | 모바일 | `sm:` | 행간 | 자간 |
|---|---|---|---|---|
| h1 히어로 | 28 | 40 | 1.3 | -0.03em |
| h2 섹션 | 24 | 28 | 1.35 | -0.02em |
| 리드(강조) | 18 | 20 | 1.5 | — |
| 본문 | 16 | 17 | 1.65 | — |
| 보조·카드 설명 | 14 | 15 | 1.6 | — |
| 배지·라벨 | 12 | 12 | 1.4 | — |

- ⚠️ **정수 px만**(`text-[16px]`). 루트가 17px라 rem 유틸(`text-sm`=14.875, `text-base`=17, `text-2xl`=25.5)은 분수 픽셀 + 표준 대비 6.25% 인플레이션. 루트 자체를 16으로 내리면 간격 유틸까지 줄어 전 페이지가 흔들리므로 금지.

#### 소개서(/m) 타입 사다리 — **폰트 밸런스의 사내 기준** (대표 확정 07-31)

> "소개서 페이지가 주는 폰트 사이즈 밸런스가 너무 좋다"(대표). 콘텐츠형 화면을 만들 땐 이 사다리에 맞춘다.

| px | 웨이트 | 역할 |
|---|---|---|
| 22 | bold | 브랜드명(상단 카드) |
| 21 | bold | 섹션 타이틀 |
| 18 | semibold | 아이템 제목(활동명 등) |
| 17 | regular | 본문(body) |
| 16 | regular | 아이템 본문(mute) |
| 15 | regular·medium | 메타·칩 |
| 13~14 | medium | 태그·캡션 |

이 사다리가 좋은 이유(재현할 것): **①bold는 꼭대기 두 단에만**(남발 금지 — medium 도배는 위계를 죽인다) **②이웃 단끼리 1px 이상 차이 + 웨이트나 색이 함께 변함** ③본문은 regular로 물러나 콘텐츠가 주인공(무대 원칙).
