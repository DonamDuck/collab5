...
| 4 | 주로 어떤 활동을 하나요? | 대표 활동을 최대 3가지 소개해주세요. 사진도 담을 수 있어요. | `activities` × {제목 · 한 줄 설명 · 사진 최대 3장}. **기본 1세트 노출 + `[+ 활동 추가]`로 최대 3까지**(링크드인 항목추가 패턴) (신규) |
...
## 타이포그래피 · 여백 (전역, 2026-07-04 추가)

- **전역 폰트 소폭 확대(디자인시스템 레버):** `globals.css`에서 루트 폰트 크기를 한 단계 올려(예: `html { font-size: 17px }`, 기본 16 → 약 +6%) rem 기반 Tailwind 사이즈(text-sm/base/lg…)가 **모든 페이지에서 일괄 확대**되게 한다. "여기 한 곳만 바꾸면 전체 반영"되는 구조.
  - 신규/개편 폼은 이 스케일에 태우기 위해 arbitrary px(`text-[15px]`) 대신 rem 기반 클래스를 우선 사용.
  - ⚠️ 전역 상향 후 주요 페이지(홈·register·검색·카드·프로필) 레이아웃 깨짐 여부 프리뷰 검증. "조금씩만" 요청이라 과하지 않게(+1px 수준).
- **섹션 간 여백 확대:** 한 섹션의 입력 영역이 끝나고 **다음 섹션 타이틀까지의 상단 margin을 넓혀** 쾌적하게. register 그룹/섹션 간격을 현재보다 한 단계 키운다(예: `space-y-8` → `space-y-12` 급, 섹션 헤더 `mt` 확대).
