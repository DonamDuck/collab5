      ...
      <div className="grid flex-1 items-center gap-5 sm:grid-cols-[1fr_46%] sm:gap-9">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-2 sm:mt-7">
            ...
            {/* ✏️08-31 대표 지시 — 「신청하기」 → **「자세히 보기」**.
                ⭐목적지가 구글폼에서 **인스타 모집 글**로 바뀌었기 때문이다(같은 날 `POST_URL` 참조).
                  누르면 «신청 창»이 아니라 «읽을 글»이 나오는데 버튼이 「신청하기」면
                  **누른 사람이 기대한 것과 나온 것이 다르다.** 신청은 그 글에서 DM으로 이어진다.
                📏실측 08-31 — 버튼 폭 **91 → 106px**(+15). 옆의 조건 문구와 한 줄을 나눠 쓰는 자리라
                  좁은 화면을 **바꾸기 전/후로 나란히** 쟀다(같은 화면에서 글자만 갈아 끼우는 방식):
                  · 375px = 둘이 **한 줄**(전·후 같음)
                  · 320px = 조건 문구가 **아래 줄로 내려간다 — 그런데 「신청하기」일 때도 그랬다**(전·후 같음)
                  👉이 변경은 **레이아웃을 안 건드린다.** 320의 줄바꿈은 원래 있던 것이고 보기에도 자연스럽다.
                🪤처음엔 「위쪽 좌표가 다르면 줄바꿈」으로 쟀는데 **버튼과 글자는 높이가 달라 원래 top이 다르다** —
                  그 판정은 375에서도 「줄바꿈」이라 나왔다(스샷은 한 줄). 아래끝/중심으로 다시 쟀다. */}
