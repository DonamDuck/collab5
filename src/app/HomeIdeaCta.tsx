...
export function HomeIdeaCta() {
  ...
  return (
    <>
      ...
      {/* 🛡️`data-cta-guard` — **플로팅 알약이 이 버튼을 덮지 않게** 하는 표식(08-17 대표 지적).
          ⭐둘 다 Kiwi가 되면서(알약 08-16 전환) 스크롤 중 **키위 알약 두 개가 위아래로 포개져** 보였다.
            게다가 **목적지가 다르다** — 여기는 분석 게이트, 알약은 `/register`. 같은 색 같은 모양인데
            누르면 다른 데로 가니 겹침이 단순한 미관 문제가 아니다.
          🔗`HomeFloatingCta`가 이 속성을 찾는다. 지우면 알약이 이 버튼 위에 그대로 앉는다. */}
      <div data-cta-guard className="mt-6 max-w-[640px] text-center sm:mx-auto">
