    ...
    screens: [
      ...
      },
      {
        ...
        rows: [
          ...
          { desc: "30분 단위 공간 · 최소 1시간(모든 공간) · 모레는 09:30~12:00 자투리, 사흘 뒤는 13:00~15:30과 16:30~18:00이 찼어요 (시작을 누르면 끝으로 고를 수 있는 칸만 켜져요) · 무료 커피챗 30분 (확인 팝업에 「무료」)", c: "guest-full", to: `/rent/${S.halfHour}` },
    ...
    screens: [
      {
        ...
        rows: [
          ...
          { desc: "알림 · 보완 요청을 받은 공간을 고쳐 다시 보냄", c: "host-full", to: "/rent/my?tab=host&saved=fixed" },
      ...
      },
      {
        title: "공간 올리기",
        path: "/rent/new",
        note: "임시 저장 복원은 링크로 못 열어요. 폼에 몇 칸 적고 새로고침하면 복원 안내가 떠요. 저장은 계정마다 따로 브라우저에 남아요. 🛍09-20부터 「대여 타입을 선택해 주세요」는 「대관만」이 켜진 채로 열려요(#119) — 상품 오류를 보려면 그걸 꺼야 하고, 켠 채로 값·설명을 비우면 그 칸 오류가 떠요. 💰수수료는 이 절 머리와 「등록 전 확인해 주세요」 두 자리에 「수수료 안내」 레몬 상자로 서요(#122·#131). 🏠「마지막으로 확인해 주세요」의 제약 사항 줄 아래 「확인했어요」를 안 누르고 「등록 신청하기」를 누르면 그 줄로 데려가요(#130. 고치기 화면엔 안 물어요). 「사업자 정보」 칸의 오류는 아래 「공간 고치기」에서 보는 게 빨라요(다른 칸이 이미 차 있어서). 🧾사업자등록증 칸에서 아무 사진이나 PDF를 고르면 목 모드라 저장소에 안 올리고 바로 글자 읽기로 가요. Gemini는 안 부르고 정해 둔 결과로 빈 칸만 채워요. 칸을 먼저 적어 두고 고르면 적어 둔 칸은 안 덮고, 등록증과 다른 칸 밑에만 「등록증에는 ○○○로 적혀 있어요」 한 줄이 떠요. 공간 주소를 비워 두고 고르면 「등록증 주소로 채우기」 제안도 떠요(넣을 때 「서울특별시」를 주소 찾기 표기 「서울」로 바꿔요).",
        rows: [
          { desc: "빈 폼 (가입 때 적은 이름·번호, 첫 소개서가 미리 채워짐)", c: "host-full", to: "/rent/new" },
          { desc: "이름·번호·소개서가 없는 사장님의 빈 폼", c: "minimal-host", to: "/rent/new" },
          // 🧾09-20 등록증 글자 읽기 — 목 결과는 세계로 갈린다(`mockBizCertRead`): 최소 입력 세계만 「못 읽음」.
          { desc: "등록증을 고르면 「등록증을 읽고 있어요…」 뒤 상호·번호·대표자·개업일이 채워지고 「올려 주신 사업자등록증을 바탕으로 아래 정보를 채웠어요」 한 줄", c: "host-full", to: "/rent/new#f-bizCert" },
          { desc: "등록증을 골라도 못 읽은 경우 「아래 칸은 직접 적어 주세요」 한 줄", c: "minimal-host", to: "/rent/new#f-bizCert" },
          { desc: "제약 사항 「확인했어요」를 안 누르고 등록 신청 (09-20 #130 — 그 줄로 데려가고 체크 밑에 빨간 말)", c: "host-full", to: "/rent/new#f-constraint" },
      ...
      },
      {
        title: "공간 고치기",
        path: "/rent/[slug]/edit",
        note: "사업자 칸 오류 보는 법 · 공개 중인 공간을 열고 「사업자 정보」에서 번호를 1234567890으로 바꿔 「고친 내용 올리기」를 누르면 번호 오류가, 대표자 이름을 지우면 빈칸 오류가, 개업일을 지우면 날짜 오류가 그 칸 밑에 떠요. 국세청 기록과 다를 때의 빨간 줄은 검토 중인 공간을 열면 처음부터 서 있고, 번호·이름·개업일 중 하나를 고치면 내려가요. 🏠주소를 바꾸면 주소 칸 밑에 「한 번 더 읽어 봐요」 줄만 떠요(09-19 저녁부터 등록증을 새로 받지 않아요. 등록증과 주소는 관리자가 검토 화면에서 견줘요). 상호만 바꾸면 검토로 안 내려가고 확인 표시도 그대로예요. 개발 서버에선 사업자등록번호 칸 밑에 「로컬 테스트: 000-00-00000」 한 줄이 떠요. 🧾등록증 칸에서 아무 사진이나 PDF를 고르면 목 결과(느린오후 로스터리 · 000-01-12347 · 김느린 · 2021년 3월 15일)와 적어 둔 값을 견줘요. 번호는 숫자만, 이름은 띄어쓰기를 빼고, 개업일은 같은 날인지로 봐서 「김 느린」처럼 띄어쓰기만 다르면 줄이 안 떠요.",
        rows: [
          { desc: "공개 중인 공간 · 상품 셋 다 켬 · 사업자 확인 승인됨 (바꾸면 표시가 내려간다는 안내)", c: "host-full", to: `/rent/${S.full}/edit` },
          { desc: "30분 단위 공간 · 09:30~12:00 자투리 날 · 매주 목요일 11:30~20:30 · 요일별 시간 아래 「시간을 다르게 할 날짜」 · 무료 커피챗", c: "host-full", to: `/rent/${S.halfHour}/edit` },
          ...
          { desc: "보완 요청을 받은 공간 · 맨 위에 「한 번 더 확인해 주세요」 사유 박스 · 버튼이 「고쳐서 다시 보내기」", c: "host-full", to: `/rent/${S.needsFix}/edit` },
          // 🧾09-20 등록증을 바꿨는데 읽은 값이 적어 둔 값과 다를 때 — 덮지 않고 그 칸 밑에 한 줄(`CertDiff`). 목 결과는 느린오후 값 하나라
          //   공간마다 적어 둔 값이 달라 세 갈래가 저절로 나온다.
          { desc: "등록증을 바꾸면 다른 칸만 한 줄 · 사업자등록번호·개업일 밑에 「등록증에는 ○○○로 적혀 있어요」와 [이 값으로 바꾸기], 빈 상호는 채워지고 같은 대표자는 말 없음", c: "host-full", to: `/rent/${S.pending}/edit#f-bizCert` },
          { desc: "등록증을 바꿔도 넷 다 적어 둔 값과 같으면 아무 말도 안 떠요", c: "host-full", to: `/rent/${S.full}/edit#f-bizCert` },
          { desc: "긴 글 · 네 칸이 다 달라 네 줄 (긴 상호 칸 밑)", c: "stress-host", to: `/rent/${S.stress}/edit#f-bizCert` },
    ...
    screens: [
      {
        ...
        title: "공간 검토",
        path: "/rent/review",
        note: "줄마다 맨 위에 「이 부분을 확인해 보세요」(국세청·주소 바뀜·네이버·예금주 신호)와 「주소 대조」(바뀐 주소 이전 → 새 · 네이버 · 등록증 보기)가 서요. 검토 대기 셋은 보완해서 다시 보냄 + 국세청 기록과 다름(공개 버튼 대신 빨간 이유 줄), 주소가 3층 → 2층으로 바뀜 + 국세청 조회 전, 계좌 예금주가 대표자와 다름이에요. 줄마다 [보완 요청]이 있고 누르면 사유 팝업(칩 넷 + 직접 쓰기)이 떠요. 아래 「열려 있는 공간 · 확인 표시 전」엔 조회가 실패한 쉬는 중 공간, 맨 아래 「보완을 기다리는 공간」엔 부탁한 날과 내용이 있어요. 목 모드라 버튼을 누르면 「저장하지 않았어요」가, 「등록증 보기」는 새 탭에 안내 문구가 떠요.",
        rows: [
          { desc: "검토 대기 셋 · 확인 표시 전 하나 · 보완을 기다리는 곳 하나 · 주소 바뀜 · 보완해서 다시 보냄 · 네이버 일치와 못 찾음 · 계좌 예금주가 대표자와 다른 곳 하나", c: "admin-full", to: "/rent/review" },
