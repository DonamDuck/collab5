...
// 🔄~~n≤12 동안 전량 노출~~(07-31) → **08-16부터 캐러셀 6개만 그린다**(`BrandGrid`의 `CAROUSEL_LIMIT`).
//   ⚠️여기서 자르지 않는 이유 — 하단 버튼이 「브랜드 **N곳** 모두 보기」로 **전체 수**를 말해야 하고,
//     칩(C2)도 전체 목록을 봐야 "실제 등록된 유형"을 가릴 수 있다. 자르는 건 화면의 일이다.
const GRID_LIMIT = 24;
const MIN_GRID = 3; // 이보다 적으면 섹션을 아예 안 그린다(디자인팀 07-27 규칙 승계).

// ⭐홈 노출 순서 — 대표가 직접 정한다(08-02). 08-16부터 **가로 캐러셀 앞 6개**가 이 순서를 쓴다.
