 ...
 *
 *  🩸🚨**「`createdAt` = 발행 시점」이라는 전제는 09-03에 깨졌다.** 08-25엔 참이었다(방혜리를 08-24에
 *    만들었고 값도 08-24). 그때는 **한자리에서 만들고 끝냈으니** 저장이 곧 발행이었다.
 *    지금 컨시어지 제작은 **행을 먼저 만들고 며칠에 걸쳐 채운다** — lmp작업실이 그랬다:
 *    행 생성 `08-31 11:39` → 마지막 수정 `09-03 11:49`, **사흘 차이**.
 *    그동안 같은 날 10분 만에 끝난 무아솔(`08-31 15:04`)이 lmp보다 **앞에** 섰다.
 *    ⭐대표 제보로 잡혔다: *「lmp작업실을 늦게 작성했는데 왜 무아솔이 맨 왼쪽에 고정되지?」*
 *    ⭐**패턴은 기억에 남고 전제는 안 남는다** — 정렬 코드는 그대로인데 «만드는 방식»이 바뀌어 뜻이 어긋났다.
 *
 *  🔧**지금 대처 = 손으로 올린다.** 공개 시점이 어긋나면 대표가 SQL로 `created_at = now()`를 준다
 *    (09-03 lmp에 실행, 1행). **설계가 아니라 임시방편이다** — 지금은 어긋남이 몇 시간~며칠이고
 *    컨시어지 건이 드물어서 이게 제일 싸다.
 *  ⛔**`updatedAt`으로 갈아타지 마라 — 더 나빠진다.** 석 달 된 소개서에서 오타 하나만 고쳐도
 *    NEW를 달고 맨 앞으로 튀어나온다. 이 섹션이 답하는 질문은 「최근에 «고친» 곳」이 아니라 「새로 «온» 곳」이다.
 *  ⏭**언제 제대로 고치나** = 「만들고 며칠 뒤 공개」가 흔해질 때. 그때 **공개 시각 칸을 따로 둔다**
 *    (`published_at` — 매거진이 이미 그 방식이다). 손으로 올리는 일이 잦아지면 그게 신호다.
