...
/** 🎠**2번 배너 슬라이드(소개서 컨시어지) 스위치.** 현재 **켜짐** — 색·문안·목업 전부 대표 확정(08-25).
 *  🔻한 번 내렸다 올린 이력이 있다(*"색상, 타이틀, 이미지 같은걸 고민하고서 하고 싶어"*). 그때 정해야 했던
 *    셋(면색·제목·그림)은 **다 정해졌다.** ⛔단 그 «값»은 여기 안 적는다 — **하루에 두 번 바뀐 값이라 또 썩는다.**
 *    👉면색·문안·목업 파일명은 전부 `HomeConciergeSlide.tsx`를 열어서 본다(값은 거기 한 곳에만 산다).
 *  ⭐끄면 캐러셀이 자동 넘김도 점도 안 만든다(`HomeBannerCarousel`의 `n < 2` 분기).
 *    ⚠️단 **1번은 카본, 2번은 연두**라 이제 두 슬라이드의 배경이 다르다 — 캐러셀은 `items-stretch`로 각 슬라이드가
 *    자기 배경을 끝까지 칠하게 둔다. `items-center`로 되돌리면 모바일에서 위아래로 검은 띠가 샌다. */
const SHOW_CONCIERGE_SLIDE = true;
    ...
    <div className="bg-surface-faint">
      ...
      >
        {/* 📝제목 교체(대표 08-16): ~~"어느 브랜드와 맞을지, AI가 먼저 찾아드려요"~~
            ...
            📝**08-31 재교체**(대표가 문장을 통째로 줌): ~~"선택한 브랜드와의 / 콜라보 아이디어를 추천해드려요."~~
              → **"마음에 둔 브랜드와의 콜라보, / 미리 그려볼 수 있어요."**
              ⭐이번에 바뀐 건 **동사**다. 「추천해드려요」는 우리가 주고 사용자는 받는 말이었는데,
                「미리 그려볼 수 있어요」는 **사용자가 해보는 말**이다. 같은 날 바꾼
                소개서 하단 [콜라보 미리 그려보기]·메뉴바 [콜라보 그려보기]와 한 벌. */}
        ...
        >
          ...
          {/* 🔻`hidden sm:inline`을 뺐다 — 대표 제보 *"모바일에서 「선택한 브랜드와의」 다음 줄바꿈이
              ...
              ✏️08-31 대표가 문장을 통째로 줬다(끊는 자리 포함) — 그대로 둔다. */}
          <br />
          미리 그려볼 수 있어요.
        </h2>
