...
//
// ⚠️ 08-02에 타이틀을 넣은 이유("회색 한 줄만 굴러 무슨 일인지 모른 채 기다린다")는 이제
//    **고정 줄이 아니라 진행 단계 자체가 받는다** — 세 단계가 읽기→접점→상상 순으로 무슨 일이
//    벌어지는지 말하고, 고정 줄은 소요시간만 맡는다. 여기에 다시 '무엇을' 설명을 얹지 말 것
//    (그러면 08-02 이전으로 돌아가는 게 아니라 같은 말을 두 줄로 하는 화면이 된다).
...
//   기존 LOADING_COPY 4개 중 "보통 30초 정도 걸려요…"만 빠졌는데, 그건 진행 단계가 아니라
//   소요시간 안내라 위 고정 줄(LOADING_SUB)로 자리를 옮겼기 때문이다 — 문구가 사라진 게 아니다.
//   구 타이틀 "콜라보 아이디어를 분석하고 있어요"도 뺐다(고정 볼드 자리 자체가 없어졌다).
const LOADING_STEPS = [
  "두 소개서를 읽는 중…",
  "접점을 찾는 중…",
  "콜라보를 상상하는 중…",
];
// 3단계 × 10s = 30s(실측 25~28초). 마지막 단계는 응답이 올 때까지 유지된다.
// ⚠️ 글자가 10초씩 머물러도 죽어 보이지 않는 건 **진행바가 계속 차기 때문**이다.
//    '살아있음' 신호를 글자 교체가 아니라 바에 넘겼으므로, 문구는 의미가 바뀔 때만 바뀐다.
//    (08-09 "전환 속도 좀 올려줘"의 답이 여기다 — 더 빨리 굴리는 게 아니라 굴릴 필요를 없앴다.)
const LOADING_STEP_MS = 10000;
