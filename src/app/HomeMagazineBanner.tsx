      ...
      >
        ...
        <div className="group grid items-center gap-5 sm:grid-cols-[1fr_46%] sm:gap-9">
          ...
          <div className="min-w-0">
            {/* 🏛️**제호(masthead)** — 대표 확정 08-16(비교 화면에서 6안 중 선택).
                📜경위 — 원래는 흰 알약 하나에 「collab5 매거진 · 창간호」를 담았는데 대표 지적이
                  *"매거진이란 게 있구나 하는 느낌이 안 든다. 그냥 배너 글처럼 보인다"*였다.
                ⭐**잡지에는 있고 우리에겐 없던 게 제호였다.** 잡지 표지는 맨 위에 잡지 이름이
                  제일 크고 기사 제목이 그 아래인데, 우리는 순서가 반대였다.
                  칩을 키우는 안(+2/+3px)도 같이 비교했지만 그건 **이름과 호수가 한 알약에 붙어
                  있는 구조**를 안 건드려서 "이 글의 꼬리표"라는 성질이 그대로 남았다.
                  → **이름은 크게, 호수는 작은 배지로 쪼갠다.** 그게 이 안의 전부다.
                🎨키위 글자 — 배너에서 유일한 브랜드색이라 눈이 여기부터 닿는다(카본 위 대비 15.64).
                  `dark`가 아닌 면(soft·kiwi)에서는 키위가 안 보이니 `primary-on`으로 떨어진다.
                📐자간 `0.06em`. 처음엔 `COLLAB5 MAGAZINE`에 `0.18em`을 줬다 —
                  **영문 대문자는 넓은 자간이 로고타입으로 읽히지만 한글은 반대다.**
                  한글은 글자 자체가 이미 네모 칸이라 벌리면 로고가 아니라 **띄어쓰기가 깨진 문장**이 된다.
                  **문자 체계가 바뀌면 자간 규칙도 바뀐다.**
                🔤한글로 간 건 대표 지시다. ⚠️내가 영문을 골랐던 이유(기사 제목과 문자 결이 갈려야
                  간판으로 읽힌다)는 사라졌으니, 위계는 **크기(17 vs 40)와 색**이 진다.
                  약해 보이면 손잡이는 셋 — 제호 17→19~20px / 배너 밖 섹션 라벨 추가 / 영문 복귀.
                🗑️같이 비교했던 B(배너 밖 라벨)·C(매거진 전체 보기)는 채택 안 됨.
                  C는 **글이 하나뿐이라** 눌러도 같은 글이고, 배너 전체가 이미 링크라 안에 링크를
                  또 두면 중첩 앵커가 된다 — 실제로 달려면 배너 링크 구조부터 바꿔야 한다.
                  글이 3편쯤 쌓이면 그때 다시 볼 것. */}
            <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
              <span
                className={`text-[15px] font-bold tracking-[0.06em] sm:text-[17px] ${
                  dark ? "text-primary" : "text-primary-on"
                }`}
              >
                collab5 매거진
              </span>
              ...
              <span
                className={`inline-flex items-center rounded-pill px-2 py-0.5 text-[11px] font-bold ${
                  dark ? "bg-white/15 text-white/80" : "bg-ink/10 text-ink"
                }`}
              >
                {isFirstIssue ? "창간호" : "최신호"}
              </span>
            </div>
