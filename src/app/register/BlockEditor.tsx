...
export function emptyBlock(type: BlockType): Block {
  ...
  switch (type) {
    ...
    case "press": return { ...base, type, items: [{ title: "", year: "", desc: "", link: "", photos: [] }] };
...
export function BlockEditor({ blocks, onChange, onUploadingChange, onSheetOpenChange, suppressFab, storyItems }: {
  ...
  const addPressItemPhotos = async (i: number, k: number, files: FileList | null) => {
    ...
    try {
      ...
      onChange((prev) =>
        prev.map((p, y) =>
          (uid ? p.uid === uid : y === i) && p.type === "press"
            ? { ...p, items: p.items.map((x, z) => (z === k ? { ...x, photos: [...(x.photos ?? []), ...urls] } : x)) }
            : p
        )
      );
    } catch { alert("사진 업로드에 실패했어요. 다시 시도해주세요."); }
  ...
  const removePressItemPhoto = (i: number, k: number, p: number) => {
    const b = blocks[i];
    if (b.type !== "press") return;
    setBlock(i, { ...b, items: b.items.map((x, y) => (y === k ? { ...x, photos: (x.photos ?? []).filter((_, z) => z !== p) } : x)) });
  };
      ...
      {blocks.map((b, i) => {
        ...
        return (
          <div key={b.uid ?? i} id={b.uid ? `block-${b.uid}` : undefined} className="space-y-4 rounded-md border border-hairline bg-surface p-3 scroll-mt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(it.photos ?? []).map((url, p) => (
                          <div key={p} className="relative h-20 w-20 shrink-0 overflow-hidden rounded-md border border-hairline">
                            ...
                            <button
                              type="button"
                              onClick={() => removePressItemPhoto(i, k, p)}
                              aria-label="사진 삭제"
                        ...
                        {(it.photos?.length ?? 0) < 3 && (
                          <label className="flex h-20 w-20 shrink-0 cursor-pointer flex-col items-center justify-center rounded-md border border-dashed border-border-strong bg-surface text-mute">
                            ...
                            <input
                              ...
                              multiple
                              className="hidden"
                              onChange={(e) => addPressItemPhotos(i, k, e.target.files)}
                            />
                          </label>
                        )}
                      </div>
            ...
            {/* ── 공통 첨부: 사진(최대3) + 링크(최대3) — press는 사진·링크 둘 다 item별로 일원화(대표 결정)라 통째로 숨김 ── */}
            {b.type !== "press" && (
              <BlockAttachments
                photos={b.photos}
                links={b.links}
