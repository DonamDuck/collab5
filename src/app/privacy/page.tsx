      ...
      <div className="mt-10 space-y-9">
        <Article title="1. 수집하는 개인정보 항목">
          ...
          <ul className="mt-2 list-disc space-y-1 pl-5">
            ...
            {/* 🏠09-17 하루 팝업 — 손님 전화번호를 신청 때 필수로 받기로 했다(대표). 결제 정보는 토스가 받고 우리는 결과만 남긴다.
                🪪09-18 — 이용하실 분 성함(실명)도 필수로 받는다(대표: 이용 당일 신분 확인). */}
            <li>
              <b>하루 팝업 이용 시</b>: 이용자 성함, 휴대전화 번호, 이용 날짜·시간·인원·이용 목적, 결제 결과(결제 수단 종류·금액·승인 번호).
              카드 번호 등 결제 수단 정보는 토스페이먼츠가 직접 받으며 회사는 보관하지 않습니다.
            </li>
            {/* 🧾09-18 공간 등록 사업자 확인(대표). 등록증은 비공개 저장소에 두고 검토하는 사람만 연다. 국세청 조회에 번호·대표자·개업일을 보낸다.
                🏷09-19 대표 [J] — 상호 칸을 새로 받고, 상호·대표자·번호를 판매자 정보 화면(`/rent/[slug]/seller`)에 공개한다. 그 사실을 같이 적는다. */}
            <li>
              <b>하루 팝업에 공간을 올릴 때</b>: 상호, 사업자등록번호, 대표자 이름, 개업일, 사업자등록증 이미지(또는 PDF).
              공간을 올린 분이 실제 사업자인지 확인하는 데 씁니다. 번호·대표자 이름·개업일은 국세청 사업자등록정보 진위확인
              서비스(공공데이터포털)에 조회하고, 등록증 파일은 공개하지 않으며 검토 담당자만 열람합니다. 상호·대표자
              이름·사업자등록번호는 사업장 주소·가게 전화번호와 함께 전자상거래법에 따라 공간 화면의 판매자 정보에 공개합니다.
            </li>
            {/* 🏦09-17 정산 계좌(대표: 사장님 도구). 원문은 서버만 읽고 화면엔 뒷자리만 나간다. */}
            <li>
              <b>하루 팝업에 공간을 올린 분이 정산 계좌를 등록할 때</b>: 예금주 이름, 은행, 계좌번호, 사업자 유형,
              사업자등록번호(개인사업자·법인만). 정산금 지급과 지급대행 판매자 등록에 씁니다.
            </li>
            <li>
        ...
        <Article title="2. 개인정보의 수집 및 이용 목적">
          <ul className="list-disc space-y-1 pl-5">
            ...
            <li>하루 팝업 예약·결제·환불 처리, 호스트와 이용자 간 연락, 거래 기록 보관, 호스트 정산금 지급</li>
            <li>하루 팝업에 공간을 올린 분의 사업자 확인(국세청 조회·등록증 검토)</li>
        ...
        <Article title="4. 개인정보의 제3자 제공">
          ...
          {/* 🏠09-17 — 하루 팝업은 거래 상대방에게 연락처를 연다. 이건 «제3자 제공»이라 따로 적어야 한다.
              🪪09-19 — 「결제가 끝나면 양쪽에」가 실제와 달랐다. 이용자 쪽 문은 결제 뒤(`guestSeesHost`), 호스트 쪽 문은 수락 뒤(`isRevealed`)다. */}
          <p className="mt-2">
            하루 팝업에서는 예약을 이행하기 위해 거래 상대방에게 아래 정보를 제공합니다.
          </p>
          <ul className="mt-2 list-disc space-y-1 pl-5">
            <li>
              <b>호스트에게</b>: 이용자의 성함·이름(브랜드명)과 이용 날짜·시간·인원·이용 목적은 결제가 끝나면, 휴대전화
              번호·이메일은 호스트가 예약을 수락한 뒤에 제공합니다. 예약 이행과 연락 목적이며, 이용일이 지나면 화면에서
              가립니다.
            </li>
            <li>
              <b>이용자에게</b>: 호스트의 이름(브랜드명)·휴대전화 번호·이메일, 매장 전화·주소. 이용자가 결제를 마친 뒤에
              제공합니다. 예약 이행과 연락 목적이며, 이용일이 지나면 화면에서 가립니다.
            </li>
          </ul>
          {/* 🧾09-18 — 공간을 올리면 사업자 진위를 국세청에 묻는다. 번호·대표자·개업일이 나간다. */}
          <p className="mt-2">
            하루 팝업에 공간을 올리면 사업자 확인을 위해 <b>국세청</b>(공공데이터포털 사업자등록정보 진위확인 서비스)에
            사업자등록번호·대표자 이름·개업일을 보내 진위와 휴업·폐업 여부를 조회합니다.
          </p>
        </Article>
        ...
        <Article title="5. 개인정보 처리의 위탁">
          ...
          <ul className="mt-2 list-disc space-y-1 pl-5">
            ...
            {/* 🧾09-20 하루 팝업 등록증 글자 읽기가 Gemini를 탄다(`lib/bizcert-ocr.ts`). 기능보다 이 줄이 먼저다(위 08-26 머리말). */}
            <li>
              Google LLC — 생성형 AI(Gemini) 처리. 브랜드 소개서 초안 생성, 콜라보 분석 리포트 작성, 하루 팝업에 공간을 올릴 때
              사업자등록증 글자 읽기(사업자 정보 칸 미리 채우기)
            </li>
            <li>토스페이먼츠 주식회사 — 하루 팝업 결제 처리, 환불, 판매자 정산(지급대행)</li>
        ...
        <Article title="5-1. 개인정보의 국외 이전">
          <p>
            ...
            해당 기능(소개서 자동 생성·콜라보 분석·사업자등록증 글자 읽기 등)의 이용이 제한될 수 있습니다.
          </p>
          <ul className="mt-2 list-disc space-y-1 pl-5">
            <li>
            ...
            <li>
              ...
              하루 팝업에 올린 사업자등록증 이미지(또는 PDF)(Google, 글자 읽기. 읽은 결과는 따로 저장하지 않고 등록 화면의 빈 칸을 채우는 데만 씁니다)
            </li>
            <li>
