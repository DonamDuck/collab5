...
//   📌시간이 기본과 다른 날은 달력에 점이 찍히고, 그 날만 아래 「시간을 다르게 할 날짜」 카드에 줄이 생긴다(09-19 #104 — 전엔 접힌 「따로 정한 날」).
//
...
export function OpenSlotsCalendar({
  ...
  /** 「시간을 다르게 할 날짜」에 «+ 날짜 추가»로 올려 둔 날짜들(09-19 #104). 시간을 아직 안 바꿔도 줄이 남아 있게 따로 든다.
   ...
   *  그래서 연 날이 하루뿐인 요일은 그날 시간을 「시간을 다르게 할 날짜」로 바꾸면 요일 시간까지 같이 바뀌었다
  ...
  /** 「시간을 다르게 할 날짜」에 서는 줄 = 따로 정한 날 + «+ 날짜 추가»로 올려 둔 날(아직 열려 있는 것만), 날짜순. */
  ...
  /** 「시간을 다르게 할 날짜」 줄 하나를 요일 시간으로 되돌린다(09-19 #104). 날짜는 그대로 열려 있다 — 닫는 건 달력이 한다.
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── ② 요일별 시간 + ③ 시간을 다르게 할 날짜 ── 달력 «아래», 고른 요일만(대표 09-16).
      ...
      {usedDows.length === 0 ? (
        ...
        <div className="space-y-2">
          ...
          {/* ③ 시간을 다르게 할 날짜 — 요일 시간과 다른 날. 바꾼 날은 달력에도 점이 찍힌다. */}
          <div className="rounded-md border border-border-strong bg-surface p-3">
            {/* 🔁09-20 대표 코멘트 #126·#127 — 제목을 묻는 말로 바꾸고, 「바꾼 날은 달력에 점으로 표시돼요」는 지웠다.
                점은 달력을 보면 알고, 여기서 말하면 할 일이 하나 더 있는 것처럼 읽힌다. */}
            <p className="text-[16px] font-medium text-ink">시간을 다르게 할 날짜가 있으신가요?</p>
            <p className="mt-1 text-[15px] leading-relaxed break-keep text-mute">
              특정한 날만 시간이 다르면 여기서 바꿔 주세요.
            </p>
...
/** 「시간을 다르게 할 날짜」의 줄 하나(09-19 #104) — 요일 줄과 같은 모양이다(날짜 칩 → 시각 둘 → 오른쪽 끝에 삭제).
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
      ...
      <button
        type="button"
        onClick={onRevert}
        // 🔁09-20 대표 코멘트 #128 — 「되돌리기」 → 「삭제」. 하는 일은 그대로다(이 줄을 지우고 요일 시간으로 돌아간다).
        //   낭독기에는 무엇이 지워지는지까지 말한다 — 「삭제」 하나만 읽히면 그날을 닫는 줄 안다.
        aria-label={`${label} 따로 정한 시간 삭제`}
