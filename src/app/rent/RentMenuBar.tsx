 ...
 *  🔁09-27 대표 D2 — *「requests로 가야할거 같아」*. 가는 곳을 `/rent/my?tab=guest`(사장님 화면의 빌린 공간 칸)에서
 *    손님 전용 `/rent/requests`로 옮겼다. 그 칸은 사장님이면서 남의 공간도 빌리는 분을 위해 남는다.
 *  ⚠️옮기면서 이 칸이 «켜질 수 있게» 됐다. 전엔 주소에 `?tab=`이 붙어 경로와 안 맞아 바가 숨었는데, 이제 `/rent/requests`에선
 *    이 칸이 「지금 여기」로 서고 바도 보인다. 「지금 여기」는 늘 한 칸이라는 규칙은 그대로다. */
const MY_TAB = { href: "/rent/requests", label: "내 예약" } as const;
