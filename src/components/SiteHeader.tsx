    ...
    <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-hairline bg-canvas px-4 sm:px-6 print:hidden">
      ...
      >
        {/* 📏 높이는 **잉크 높이**다 — 08-16에 로고 SVG의 viewBox를 잉크에 맞게 조였다.
            ...
            🪤 폭 예산 — 잉크 종횡비 5.78이라 높이 1px = 폭 5.8px다. 360px 폰의 `/search`
               헤더(로고+검색칩+로그인)가 제일 빡빡해서 여기가 상한을 정한다. 21px일 때
               로고 오른쪽에 17px 남는다(실측). 더 키우려면 먼저 그 화면부터 재라.
            🪤 08-16 로고 교체로 **마크가 글자보다 위아래로 더 나온다**(마크 77.7 : 글자 71.8).
               그래서 같은 글자 크기를 유지하려면 박스가 20 → 21이어야 한다. 20/28을 그대로
               두면 글자가 되레 작아진다 — 박스 높이를 글자 높이로 착각하지 말 것. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo-lockup.svg" alt="" aria-hidden="true" className="h-[21px] w-auto sm:h-[30px]" />
      </Link>
