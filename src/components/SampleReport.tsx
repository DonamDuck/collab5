    ...
    <div className="max-w-[640px]">
      <div className="rounded-xl border border-hairline bg-surface p-5 text-left shadow-e1 sm:p-7">
        {/* 식별 캡션 — 예시임을 정직하게 밝힌다(과장 금지).
            ...
            🔻08-16 ①한 줄 12px 회색 텍스트 → ②칩 + 브랜드쌍 두 덩어리 → ③**칩 하나로 합침**(대표 지시).
              ⭐②에서 ③으로 간 이유 — 칩("실제 소개서로 만든 예시")과 브랜드 쌍이 **따로 놀았다.**
                칩은 성격을, 아래 볼드는 대상을 말하는데 둘이 서로를 안 가리켜서 두 번 읽어야 했다.
                문장 하나에 **어디 소개서인지(collab5에 등록된) + 무엇인지(리포트 예시)**를 다 넣으니
                한 번에 끝난다. 브랜드 쌍은 그 아래 볼드로 남긴다 — 실제 쌍을 밝히는 건
                "가짜 예시 금지" 원칙(07-31)의 핵심 신뢰 신호라 지울 수 없다.
        ...
        <span className="inline-flex rounded-md bg-surface-soft px-2.5 py-1.5 text-[12px] font-medium break-keep text-mute">
          collab5에 등록된 소개서로 분석해본 리포트 예시예요.
        </span>
        <p className="mt-2.5 break-keep text-[15px] font-bold text-ink">
          {sampleData.fromName} <span className="text-faint">×</span> {sampleData.toName}
        </p>
