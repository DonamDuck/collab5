...
- descriptions 6개 (각 3~5문장, 어미는 아래 '어미 리듬' 규칙):
  ...
  · ⭐⭐마무리 규칙(핵심) — 착지점은 세 종류가 있고, **셋을 구분하지 못하면 글이 얇아진다**(대표 직접 퇴고 08-02에서 확립).
    ...
    ⚠️모든 후보를 [3]으로 끝내 리듬이 똑같아지는 것도 피해라 — 6개 중 **최소 3개 이상**이 [3]으로 닫히면 된다.
  ...
  · ⭐6번째 = '사실 정리형'(다른 5개와 성격이 다르다) — 감상·서사를 빼고, 조사 자료에서 확인된 구체 사실을 밀도 높게 담아라. 위치·규모·전공/이력·프로그램 종류·공간 성격·이용 방식처럼 '이 브랜드가 무엇인지'를 사실로 빠르게 파악되게. 다른 후보보다 정보 밀도가 눈에 띄게 높아야 한다. ⛔단 영업시간·휴무·가격은 사실 정리형에도 금지(위 '유통기한 있는 정보 금지' 규칙 적용).
...
⛔출력 직전 자기검열 — 두 번 훑어라.
[1차: 상투어] descriptions 6개 각각에 다음이 한 단어라도 있으면 그 문장을 구체 사실로 다시 써라: 믿습니다 / 응원합니다·응원해요·지지합니다 / 빛나는 / 든든한 / 소중한 / 특별한 / 함께 성장 / 꿈(을 향해·을 펼치다) / 첫걸음·시작점 / 긍정적인 에너지 / 새로운 내일·미래. 특히 각 후보의 마지막 문장이 "~응원합니다 / ~바랍니다"류 추상 격려로 끝나면 안 된다. 이건 warmth를 죽이라는 게 아니라, 구체에 뿌리내리지 않은 빈 감정어만 걷어내라는 것이다(담백한 온기는 사실 속에서 배어난다).
[2차: 페이오프 — ⚠️descriptions에만 적용] ⭐**descriptions 6개의 마지막 문장만 따로 모아서 읽어라.** 전부 사실 진술로 끝나 있으면 그 글은 얇은 것이다 — 최소 3개는 위 '마무리 규칙 [3] 의미로 닫기'로 고쳐 써라. ⛔단 6개 전부를 그렇게 닫지는 마라(리듬이 죽는다).
