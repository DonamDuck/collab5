...
⭐사실을 적고 거기서 멈추지 마라(대표 퇴고 08-02) — 한 항목의 마지막 문장은 가능하면 **그래서 무엇이 생겼는지**로 닫는다: "…편하게 대화하는 자리를 만들었어요" / "…이웃과 연결되는 시간을 만들었어요". 단 이건 추상 응원("빛나는·함께 성장")으로 닫으라는 말과는 정반대다 — **그 자리에서 실제로 생긴 것**만 이름 붙인다. 근거가 없으면 사실로 끝내라.
...
- activityHints: 조사 메모에 실제로 언급된 이 브랜드의 활동만 0~5건.
  ...
  ⭐activityHints·collabHints의 desc도 위 **'마무리 규칙'을 그대로 따른다** — 무엇을 했는지 적고 멈추지 말고, 근거가 있으면 그래서 무엇이 생겼는지로 닫아라. "매번 새로운 주제를 정해 진행했어요"(사실에서 멈춤) → "매번 새로운 주제로 모여, 오며가며 만난 사람들이 편하게 이야기하는 자리를 만들었어요"(의미로 닫음). ⚠️근거가 없으면 억지로 의미를 붙이지 말고 사실로 끝내라 — 지어낸 의미는 상투어와 같다.
...
[2차: 페이오프] ⭐**각 후보의 마지막 문장만 따로 모아서 읽어라.** 전부 사실 진술로 끝나 있으면 그 글은 얇은 것이다 — 최소 3개는 위 '마무리 규칙 [3] 의미로 닫기'로 고쳐 써라. 같은 검사를 activityHints·collabHints의 desc에도 적용한다.`;
