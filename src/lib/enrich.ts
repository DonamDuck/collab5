...
[제품·서비스] 주력 제품·서비스의 이름·종류 (각 한 줄). 업종에 맞게: 요가원이면 프로그램(하타·빈야사·테라피), 송금·IT면 서비스 라인(개인 송금·기업 결제·API), 카페면 대표 메뉴·원두, 공방이면 제품군·클래스
  ⭐그 업계에서 통용되는 **분류·유파·기법 이름이 자료에 있으면 반드시 그대로 적어라** (요가=하타·빈야사·아쉬탕가·인요가 / 커피=싱글오리진·블렌드·핸드드립·에스프레소 / 공예=업사이클·패치워크·마크라메 / 미용=옴브레·클리닉). 이 이름들이 브랜드끼리 비교하는 기준이 된다. ⛔단 자료에 없으면 지어내지 마라 — 없는 유파를 붙이면 거짓이 된다.
...
[활동] 워크숍·클래스·제품 라인 등 **이 브랜드가 스스로 하는 일** (각각 무엇이었는지)
  ⛔상대가 있어야 성립하는 일은 여기 쓰지 말고 [콜라보]로 보내라 — 협업·공동제작·입점·팝업 참여·페어 참가·외부 기관 수업.
  판별: "누구와 함께"가 붙어야 말이 되면 콜라보다. 그 상대가 없어도 늘 하는 일이어야 활동이다.
[콜라보] 다른 브랜드·공간·기관·작가와 함께한 협업 이력 (파트너 이름이 확인된 것만)
[원하는 협업] 이 브랜드가 찾는 파트너·하고 싶다고 밝힌 협업 (모집글·인터뷰 발언 등 근거 필수)
  ⛔⛔"받지 않습니다 / 사양합니다 / 정중히 거절합니다"처럼 **안 받겠다고 밝힌 문의 유형은 절대 여기 쓰지 마라** — 정반대 뜻이 된다. 그런 문장은 통째로 건너뛴다(예: "유료 홍보·체험단 문의는 받지 않습니다" → 아무것도 적지 않는다).
[고객] 주요 고객층·타겟 — ⭐**사람 집단만** 적어라(예: 임산부, 30대 직장인, 지역 주민, 반려인, 초보자, 관광객). 관심 주제·감정·증상·수업 내용(예: 마음공부·불안·유연성·힐링)은 고객이 아니다 → 그건 [제품·서비스]나 [특장점]에 적어라. 확인된 것만.
...
⭐인스타 개별 게시물 — 검색 결과에 이 브랜드 계정의 개별 게시물 URL(instagram.com/p/… 또는 …/reel/…)이 보이면 그 URL을 원문 그대로 나열해줘(최대 5개, 줄마다 하나). 사장님이 직접 쓴 글이라 소개서 재료로 가장 값지다. 다른 계정 게시물은 넣지 마.
