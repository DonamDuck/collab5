...
export interface DraftInput {
  ...
  pressDigest?: string; // 보도기사 본문 발췌 — 서버(fetchArticleExcerpts)에서만 생성
...
export interface OptionsInput {
  ...
  homepageDigest?: string; // 홈페이지 딥리드 발췌 — 서버(fetchHomepageDigest)에서만 생성, 클라이언트 텍스트 금지
  instagramDigest?: string; // 인스타 발췌(바이오·캡션 = 사장님이 직접 쓴 글) — 서버(fetchInstagramDigest)에서만 생성
  pressDigest?: string; // 보도기사 본문 발췌 — 서버(fetchArticleExcerpts)에서만 생성. '소개된 곳' 설명 보강용
}
...
- ⭐⭐지금 하고 있는 일·현황은 '계속되고 있음'이 느껴지는 현재형으로 쓴다: ~하고 있어요 / ~이어가고 있어요 / ~해오고 있어요 / (혼합체 문장이면) ~하고 있습니다 · ~만들어 가고 있습니다 · ~넓혀 가고 있습니다.
  🚨**과거형만 막으면 부족하다 — '완결형'도 똑같이 막아라**(대표 지시 08-02). "만듭니다 / 완성했습니다 / 구축했습니다 / 마련했습니다 / 이루어 냈습니다"처럼 **한 번에 끝낸 것처럼 들리는 서술어**는 브랜드가 멈춘 것으로 읽힌다. 아직 이어지는 일이면 반드시 **진행형**으로 바꿔라:
  · ⛔"…실질적인 계기를 **만듭니다**" → ⭕"…실질적인 계기를 **만들어 가고 있습니다**"
  · ⛔"…불안을 확신으로 바꿔주는 가이드를 **완성했습니다**" → ⭕"…가이드를 **완성해 가고 있습니다**" / "**만들어 가고 있습니다**"
  · ⛔"…포용적 문화를 **정착시켰습니다**" → ⭕"…**넓혀 가고 있습니다**"
  ⚠️**단 정말 끝난 사건은 과거형이 맞다** — 지난 콜라보·일회성 행사·수상은 "~했어요 / ~열었어요"로 닫는다. 판정 기준은 어미가 아니라 **그 일이 지금도 이어지는가**다.
  ⚠️문단을 닫는 마지막 문장에서 특히 자주 샌다. **닫기 전에 "이 일이 지금도 계속되나?"를 한 번 묻고 서술어를 고르라.** — "2013년에 시작했어요"(X, 끝난 느낌) → "2013년에 시작해 지금까지 이어가고 있어요"(O). 과거형은 정말 끝난 일(지난 콜라보·행사 이력)에만.
...
⭐**긴 글(브랜드 소개·시작 이야기)에 한해** — 사실을 적고 거기서 멈추지 말고, 마지막 문장은 가능하면 **그래서 무엇이 생겼는지**로 닫는다: "…편하게 대화하는 자리를 만들었어요"(대표 퇴고 08-02). 단 이건 추상 응원("빛나는·함께 성장")으로 닫으라는 말과는 정반대다 — **그 자리에서 실제로 생긴 것**만 이름 붙인다. 근거가 없으면 사실로 끝내라.
🚨⛔**짧은 필드에는 위 규칙을 적용하지 마라** — 활동·콜라보의 "desc", 각종 "note"·힌트는 **한두 문장 사실 기술**이 정답이다. 서술어를 "~하고 있어요"·"~했어요"로 딱 잘라 끝낸다. 짧은 칸에 페이오프를 욱여넣으면 글이 길어지고 일기처럼 읽힌다(08-02 실제 사고).
⭕**닫는 정답 문형 = 형식을 명명한다** — "[내용 서술] + [○○ 행사·모임·프로그램·클래스·팝업·장터·콜라보]를 진행했어요 / 운영하고 있어요". 예: "…다양한 장르를 즐겨보는 New Jack Saturday 행사를 진행했어요".
⛔**감상의 명사화만 금지** — "~하는 자리를/시간을 만들었습니다". **형식(행사·모임·프로그램)은 분류라 단단하고, 감상(자리·시간)은 붕 뜬다.** 둘을 헷갈리지 마라.
⭐사람이 문장 가까이 있게 — 공간·제품·재료만 나오는 문단은 정확해도 차갑다. 사람들이·이웃과·손님들과가 자연스럽게 등장하게 쓴다.
⭐⭐맞춤법·띄어쓰기 (출력 전 반드시 자기 점검) — 소개서는 사장님이 남에게 보여줄 글이다. 오탈자 하나가 신뢰를 깎는다.
· 의존명사는 띄어 쓴다: 할수(X)→할 수 / 만들수(X)→만들 수 / 하는것(X)→하는 것 / 할때(X)→할 때 / 이곳뿐아니라(X)→이곳뿐 아니라.
· 보조용언·연결어를 붙이지 마라: 해오고있어요(X)→해오고 있어요 / 만들어가고있습니다(X)→만들어 가고 있습니다.
· 조사는 앞말에 붙여 쓴다: 저희 는(X)→저희는 / 공간 을(X)→공간을.
· 자주 틀리는 것: 됬어요(X)→됐어요 / 되요(X)→돼요 / 몇일(X)→며칠 / 오랫만에(X)→오랜만에 / 갯수(X)→개수 / 어떡해(감탄)와 어떻게(방법) 구분 / 로서(자격)와 로써(수단) 구분 / 안(부정)과 않(하지 않다) 구분.
· 문장부호: 마침표 앞에 공백을 두지 말고, 공백은 한 칸만. 말줄임은 …가 아니라 생략하고 문장을 끝내라.
· 브랜드명·메뉴명 등 고유명사는 조사 자료의 표기를 그대로 따르되, 그 외 일반 어휘는 표준어로 쓴다.
⭐⭐유통기한 있는 정보 금지 (2026-07-21 대표 확정) — 소개서는 오래 보는 문서다. 시간이 지나면 틀려지는 정보는 쓰지 마라.
· ⛔영업시간·휴무 요일은 어디에도 쓰지 마라(자세히 소개·공간·활동 전부). 시간 안내는 소개서의 지도 링크가 담당한다.
· ⛔구체 가격("23,000원"), 기간 한정("7/24까지", "이번 달"), 선착순·재고성 문구 금지. 단 반복 패턴으로 일반화는 허용: "매달 새로운 기획전을 열고 있어요"(O) / "7월 뽀모리 샴페인 행사 중"(X).
· ⭐활동 자격은 '반복'이 아니라 **'그 브랜드다움이 드러나는가'**다(대표 08-02 개정 — 07-21의 "반복되는 것만"은 폐기). 정기 프로그램·대표 메뉴·상시 서비스가 1순위지만, **일회성 팝업·행사·프로그램도 브랜드의 가치와 매력이 보이면 활동으로 쓴다.** 반복 여부는 자격 요건이 아니라 **우선순위 가중치**다.
· 단 일회성을 쓸 땐 여전히 **날짜·가격·마감은 빼고** 무엇을 왜 했는지로 쓴다. 연도·계절("2025년 여름에 열었어요")은 지나도 틀려지지 않으니 허용.
⭐사장님이 직접 쓴 글(인스타 바이오·캡션, 예약 플랫폼 소개문)이 자료에 있으면:
· 그 표현(애칭·슬로건·"낮술·혼술 환영" 같은 원문 단어)을 최대한 원형 보존해 반영하라. 제3자 문서(기사·블로그)와 사실이 충돌하면 사장님 글을 따른다.
· 팔로워 수 같은 숫자는 자랑이 되는 규모일 때만 언급하고, 작은 숫자는 쓰지 마라.`;
...
const OptionsResultSchema = z.object({
  ...
  activityHints: z
    .array(
      z.object({
        title: z.string().describe("짧은 활동명 — 고유명이 있으면 그대로 쓰고 뒤에 형식어(팝업·모임·클래스·장터·프로그램·생산 등)를 붙여 제목만 봐도 무엇인지 알게 한다. 할인·프로모션 명칭은 넣지 않는다"),
        desc: z.string().describe("한두 문장 요약, 해요체"),
        source: z.string().describe("출처 유형: 네이버 블로그 후기/카페글/웹 검색/인스타그램 중 하나"),
      })
    )
    .describe("조사 메모에 실제 언급된 활동 흔적 0~5건. 없으면 빈 배열"),
  collabHints: z
    .array(
      z.object({
  ...
  properties: {
    ...
    activityHints: {
      ...
      items: {
        type: Type.OBJECT,
        properties: {
          title: { type: Type.STRING, description: "짧은 활동명 — 고유명이 있으면 그대로 쓰고 뒤에 형식어(팝업·모임·클래스·장터·프로그램·생산 등)를 붙여 제목만 봐도 무엇인지 알게 한다. 할인·프로모션 명칭은 넣지 않는다" },
      ...
      description: "조사 메모에 실제 언급된 활동 흔적 0~5건 — 대표 메뉴·정기 프로그램·상시 서비스·워크숍·클래스·제품라인 등이 1순위지만, 브랜드다움이 드러나면 자체 기획 팝업·행사 같은 일회성도 포함한다. ⛔협업·공동제작·입점처럼 상대가 있어야 성립하는 일은 여기가 아니라 collabHints. 메모에 없으면 빈 배열 — 창작 금지",
    },
    collabHints: {
      type: Type.ARRAY,
      items: {
...
작성 규칙:
- oneLiners (개수는 요청문이 지시 · 각 20~40자):
  ...
  · ⭐**지역·범위는 완전하게**(대표 퇴고 08-06) — 사업 지역·분야가 여럿이면 일부만 쓰지 말고 전부 담아라. "세부 자유여행"(X, 보홀·발리 누락) → "세부·보홀·발리 자유여행"(O). 조사 메모에서 확인된 범위 기준.
  · 앵글 풀(요청 개수만큼 골라 쓴다): ①핵심 제품·방식(가장 구체) ②철학·태도를 구체 행동으로 ③고객이 얻는 경험 ④숫자·이력 근거(조사에 있을 때만, 없으면 다른 각도) ⑤협업 파트너가 끌릴 지점.
  · ⭐슬로건 앵글(조건부) — 조사 자료에 이 브랜드의 실제 슬로건·태그라인이 있을 때만, 후보 중 하나를 [슬로건 원형] — [하는 일 한 줄] 형식으로 만들어라. 예: "발길 닿는 곳마다, 진정한 로컬의 경험 — 세부·보홀·발리 자유여행을 만들어가고 있어요". 세부 규칙: ①구분자는 em-dash "—" 고정(하이픈 "-"·물결 "~" 금지). ②앞부분은 사장님이 실제로 쓰는 슬로건을 원형 그대로 보존(표현 변형·의역 금지) — verbatimKeywords가 1순위 소스, 그다음 홈페이지 태그라인·히어로 문구, 인스타 바이오 첫 줄 순서로 찾아라. ③뒷부분은 이 브랜드가 구체적으로 하는 일을 사실 기반 해요체로(앞이 감성이면 뒤는 사실로 균형 — 앞뒤 둘 다 감성이 되지 않게). ⛔단 조사 자료(verbatimKeywords·홈페이지 발췌·인스타 발췌)에 실제 슬로건·태그라인이 없으면 이 앵글을 만들지 마라 — 슬로건을 지어내는 것은 절대 금지다(가짜 슬로건은 없느니만 못하다). 슬로건 소스가 없으면 이 앵글은 생략하고 위 ①~⑤ 기존 앵글로 채워라.
  · 형용사를 줄이고 명사로 말하라. "감성적인 소품"(X) → "폐원단으로 짓는 파우치"(O).
  · ⭐⭐**"무엇을 파는가"보다 "사람들이 왜 여기 머무는가"**(대표 지시 08-02). 한 줄 소개는 판매 품목 나열이 아니라 **이 브랜드가 만드는 경험**이 먼저 보여야 한다. 취급 품목을 이어 붙인 문장은 정확해도 매력이 없다.
    ⛔"좋은 커피와 음악, 그리고 굿즈를 판매하고 있어요"(품목 나열) → ⭕"성북천 곁에서 좋은 커피와 음악, 사람들의 이야기가 머무는 공간을 만들고 있어요"(머무는 이유).
    ⚠️읽는 사람은 **콜라보 상대**다. 이 문장을 읽고 "여기랑 뭔가 같이 해보고 싶다"가 들어야 한다 — 그 판단은 품목이 아니라 경험·결에서 나온다. 단 경험을 감성어로 뭉개지 말고 구체 명사(장소·재료·활동)에 얹어라.
- descriptions (개수는 요청문이 지시 · 각 3~5문장, 어미는 아래 '어미 리듬' 규칙):
  ...
  · ⭐⭐어미 리듬 = 문장의 역할이 어미를 정한다(기능 기반 혼합체). 믿음·주장·확정 선언(뼈대)은 "~합니다"("자신을 표현하는 방법이라고 생각합니다" / "취향과 표현을 발견합니다"), 이유·부연·숨 고르기(연결)는 "~이에요/~예요"("이야기가 담기기 때문이에요"), 바람·지향(마무리)은 차분한 "~바랍니다". ⚠️합니다만 연속=사보처럼 딱딱 / 에요만 연속=수다처럼 가벼움 — 선언 사이에 부연이 끼며 숨 쉬는 리듬으로 섞어라. 애교체(~한답니다·~하고 있죠·~거든요)는 지양(조용한 확신이 기준). 과한 이모지·느낌표 금지.
    ⭐**문장 길이 리듬**(대표 퇴고 08-06) — 짧은 문장 3~4개가 연달아 뚝뚝 끊기면 사보처럼 읽힌다. **그중 한 문장 정도는 쉼표나 '~하고 있고,'로 이어** 긴 문장 하나를 만들라. ⛔연결 어미 '~있고요/~고요'는 쓰지 마라(대표 금지) — 이을 거면 문장 중간에서 '~하고 있고,'로 잇고, 끝낼 거면 '~있어요'로 끝낸다.
    ⛔**'닫다' 은유 금지**(대표 퇴고 08-06 "어색하다") — 본문 문장 안에서 "하루를 닫는 / 여행을 닫는" 같은 표현을 쓰지 마라. "하루를 마무리하는"처럼 평이한 동사로 쓴다.
  · ⭐Why가 한 번은 보이게 — 후보 전체에서 "왜 이 일을 하는가"(브랜드가 믿는 가치)가 최소 한 번은 드러나야 한다. 단 Why도 조사 메모의 구체에 뿌리내려야 하며("정성을 담습니다"류 어디에나 붙는 철학 금지), 모든 후보를 "우리는 ~을 믿습니다. 그래서"로 시작하는 템플릿 반복은 금지 — 철학·시작스토리 앵글에 강하게, 나머지 앵글은 활동→경험 흐름으로.
  ...
  · ⭐**브랜드의 자기 어휘를 우선하라**(대표 퇴고 08-06) — 브랜드가 스스로 쓰는 말을 일반어로 번역하지 마라. 브랜드가 '로컬'을 정체성으로 쓰면 '지역 아티스트'가 아니라 '로컬 아티스트'다. 조사 메모의 원문 어휘를 살린다.
  · ⭐⭐마무리 규칙(핵심) — 착지점은 세 종류가 있고, **셋을 구분하지 못하면 글이 얇아진다**(대표 직접 퇴고 08-02에서 확립).
    ⛔[1] 추상 응원·격려·바람 — "빛나는 미래를 응원합니다 / ~되기를 바랍니다". 아무 브랜드에나 붙는 빈 문장. 절대 금지.
    △[2] 사실로 닫기 — "매주 목요일 저녁, 함께 책을 읽습니다". 틀리진 않지만 **정보가 거기서 멈춘다.** 허용은 하되 이걸 기본값으로 삼지 마라.
    ⭕[3] 의미로 닫기(목표) — 무엇을 했는지 말한 뒤 **그래서 그게 무엇이 되었는지**로 닫는다. "오며가며 만난 사람들이 편하게 대화하는 자리를 만들었습니다" / "이웃과 자연스럽게 연결되는 시간을 만들었습니다".
    핵심 구분: [3]은 [1]처럼 감정어를 얹는 게 아니라, **그 자리에서 실제로 생긴 것**(자리·시간·계기·관계)을 이름 붙여 말하는 것이다. 근거 없는 의미 부여는 [1]로 미끄러지니, 앞 문장들의 구체 사실에서 자연스럽게 따라 나오는 것만 써라.
    ⚠️모든 후보를 [3]으로 끝내 리듬이 똑같아지는 것도 피해라 — **후보의 절반 이상**이 [3]으로 닫히면 된다.
    🚨⭐**[3]의 시제는 그 일이 끝났는지에 따라 갈린다**(대표 지시 08-02 — 여기서 제일 많이 샌다).
      · **끝난 사건**(지난 콜라보·일회성 행사) → "…편하게 대화하는 자리를 **만들었습니다**" ⭕
      · **지금도 이어지는 일**(상시 서비스·정기 프로그램·브랜드가 계속 쌓는 것) → "…**만들어 가고 있습니다** / **넓혀 가고 있습니다**" ⭕
      · ⛔**둘을 헷갈려 완결형으로 닫으면 브랜드가 멈춘 것처럼 읽힌다** — "가이드를 완성했습니다"·"계기를 만듭니다"(X).
      위 예시의 "만들었습니다"만 보고 그 문형을 복사하지 마라. **닫기 전에 "이 일이 지금도 계속되나?"를 묻고 서술어를 고를 것.**
  · ⭐**사람이 문장 가까이 있어야 한다** — 공간·제품·재료만 나오는 문단은 정확해도 차갑게 읽힌다. 사람들이·이웃과·손님들과·참가자들은 처럼 **그 자리에 있는 사람**이 각 후보에 최소 한 번은 등장하게 하라(단 "고객님께 최고의"류 서비스 멘트는 금지 — 사람은 주어·동반자로 등장하지 대상으로 등장하지 않는다).
  ...
  · ⭐⭐시작 스토리 앵글(앵글 1) 작성법(대표 지시 07-31) — **사건을 시간순으로 나열하지 마라.** 브랜드를 가장 빨리 이해시키는 흐름으로 재구성한다: ①왜 시작했는가 → ②무엇을 깨달았는가 → ③그 깨달음을 이름·철학에 어떻게 담았는가 → ④지금 무엇을 만들고 있는가. 넷을 다 쓸 필요는 없고, 조사 메모에 재료가 있는 것만 이 순서로 잇는다. 원문의 핵심 메시지는 지키되 **문장은 새로 써라**(인터뷰 발화·홈페이지 문장의 통짜 이식 금지 — 대표 판정 "원문 그대로면 오글거린다"). 같은 뜻을 표현만 바꿔 두 번 말하지 마라. ⭐**에세이가 아니라 브랜드 소개글로 읽혀야 한다** — 회고·심경 묘사를 길게 늘이지 말고 다른 앵글과 같은 3~5문장 안에서 닫는다.
  · ⭐마지막 하나 = '사실 정리형'(다른 후보와 성격이 다르다) — 감상·서사를 빼고, 조사 자료에서 확인된 구체 사실을 밀도 높게 담아라. 위치·규모·전공/이력·프로그램 종류·공간 성격·이용 방식처럼 '이 브랜드가 무엇인지'를 사실로 빠르게 파악되게. 다른 후보보다 정보 밀도가 눈에 띄게 높아야 한다. ⛔단 영업시간·휴무·가격은 사실 정리형에도 금지(위 '유통기한 있는 정보 금지' 규칙 적용).
...
- activityHints: 조사 메모에 실제로 언급된 이 브랜드의 활동만 0~5건.
  ⭐**title 규칙 — 제목만 봐도 "이게 무엇인지" 알게 하라**(대표 실제 편집에서 증류, 08-02).
  사장님이 붙인 고유명이 있으면 **그대로 살리고 뒤에 형식어**(팝업·모임·클래스·장터·프로그램·생산·행사)를 붙인다.
  ⭕"우리집 살구나무 **팝업**" · "세나와 일본어로 인사하기 **모임**" · "쏠티팬트리 - 캔 통조림 **직접 생산**" · "매달 오픈하는 캔앤코르크 와인 **장터**"
  ⛔고유명만 덩그러니("우리집 살구나무") — 무엇인지 모른다. ⛔**할인·프로모션 명칭은 제목에 넣지 마라**("도전! 전국최저가" 같은 것은 뺀다. 소개서 격에 안 맞는다).
  ⭐⭐**제목은 담백형 한 가지다**(대표 확정 08-06) — 구조 = **[누구와/누구를 위한 · 지역] + [무엇] + 형식어**.
  ⭕"가이드와 함께하는 세부·보홀 정기 청소 프로그램" · "여행객을 위한 세부·보홀 베이스캠프 운영"
  ⛔**제목에 감성·의미 부제를 달지 마라** — "— 여행자의 쉼을 직접 지은 공간" · "— ~을 지키는 일" 같은 꼬리는 광고 카피처럼 읽힌다(대표가 실제로 걷어냄). **'왜'는 desc 본문의 몫이고 제목은 분류다.**
  ⭐형식어는 **변주하라**(운영·제작·진행·프로그램·클래스·생산·투어·장터) — 활동 여러 개가 전부 "~운영"으로 끝나면 단조롭다.
  ⭐**기준은 '반복되는가'가 아니라 '이 브랜드다움이 드러나는가'다**(대표 08-02 개정). 대표 메뉴·정기 프로그램·상시 서비스·워크숍·클래스·제품라인·매달 반복되는 기획이 **1순위**지만, **자체 기획 팝업·계절 메뉴·일회성 행사도 브랜드의 가치와 매력이 보이면 활동으로 올려라.** 반복은 자격 요건이 아니라 **순위 가중치**다(정기 > 일회성).
  ⛔단 **유통기한 정보를 빼고 나면 남는 게 없는 것**(날짜·가격·마감 공지뿐인 글)은 올리지 마라. 일회성을 쓸 땐 날짜·가격·마감을 빼고 무엇을 왜 했는지로 쓰되, 연도·계절은 써도 된다("2025년 여름에 열었어요").
  음식점·바라면 대표 메뉴와 즐기는 방식도 훌륭한 활동이다 — 소극적으로 굴지 말고 메모에 근거가 있으면 적극 추출해라.
  ⭐⭐**desc는 '한두 문장 사실 기술'이다 — descriptions와 성격이 완전히 다르다**(대표 지시 08-02, 실제 사고에서 확립).
  · **길이 = 한두 문장, 최대 세 문장.** 넘으면 무조건 줄여라. 이 필드는 읽는 사람이 훑는 목록이지 읽는 글이 아니다.
  · **서술어는 행위로 닫는다** — "~하고 있어요" / "~하고 있습니다" / "~했어요"(끝난 일). 지금 하는 일은 현재형, 끝난 일은 과거형으로 **딱 잘라** 쓴다.
  · ⭕⭐⭐**닫는 정답 문형 = 형식을 명명한다**(대표가 실제로 쓴 문장에서 증류, 08-02). 마지막을 **[내용 서술] + [○○ 행사·모임·프로그램·클래스·팝업·장터·콜라보]를 진행했어요 / 운영하고 있어요** 로 닫아라. "이게 무엇인지" 이름을 붙이며 끝내는 것이다.
    ⭕"세 명의 DJ와 함께 뉴잭스윙, 디스코, 소울 하우스 등 다양한 장르를 즐겨보는 New Jack Saturday **행사를 진행했어요**"
    ⭕"…이 달에 추천할 와인리스트를 구성해 월간 장터 **프로그램을 운영하고 있어요**"
    ⭕"…산책과 식사를 나누며 이웃과 연결되어 보는 **콜라보를 진행했어요**"
  · ⛔**감상의 명사화는 금지**(가장 자주 나는 사고) — "~하는 **자리를/시간을** 만들었습니다". 같은 명사로 닫아도 **형식(행사·모임·프로그램)은 분류라 단단하고, 감상(자리·시간)은 붕 뜬다.** 둘을 헷갈리지 마라.
    ⭕"한 캔의 경험을 함께 즐기는 시간으로 **확장하고 있습니다**"(동사로 담음) ⛔"…캔을 곁들이는 **시간을 만들었습니다**"
  · ⛔**서사·회고·감정 서술 금지** — "그 한 입이 오래 남았고", "어찌나 잘 그리시는지" 같은 문장은 이 필드에 넣지 마라. 그건 descriptions·story의 몫이다.
  · ⛔**자기 해설 꼬리 금지**(대표 퇴고 08-06 "안 어울린다") — 사건 서술 뒤에 의미를 해설하는 문장("투어 너머 머무는 시간까지 여행의 일부로 담으려는 **시도였습니다**" / "~하려는 **마음이었어요**")을 붙이지 마라. 이 필드는 사실 문장으로 끝나도 충분하다 — '의미로 닫기'는 descriptions 전용이지 활동·콜라보 desc의 규칙이 아니다.
  · ⛔**자료에 없는 장면을 지어내지 마라** — 페어링 행사를 "한 잔마다 굴 한 입을 맞춰보는 자리"로 부풀리는 식. 캡션에 있는 사실만.
  · **collabHints는 더 짧게(1~2문장)** — 무엇을, 누구와, 어떻게 했는지면 충분하다.
  ⚠️위 '마무리 규칙(의미로 닫기)'은 **descriptions 전용**이다. 이 필드에 적용하지 마라 — 한두 문장 안에 사실과 페이오프를 다 넣으려다 글이 길어지고 일기처럼 된다.
...
- offersHint: seeksHint(원하는 파트너)와 별개로, 이 브랜드가 파트너에게 '제공할 수 있는' 협업의 초안. 콜라보 이력·공간·활동·제품에서 유도해라 — 사장님들이 가장 낯설어하는 항목이라 맞춤 초안이 특히 값지다. types 최대 4개(같은 유형 목록에서), note는 한두 문장: ⭐해본 것은 과거형으로("와인 시음 행사를 함께해 봤어요"), 앞으로 열려 있는 것은 현재형으로("페어링 팝업도 열려 있습니다") — 이력이 없으면 공간·제품에서 가능한 것만 담백하게. ⭐아직 안 해본 협업은 **가능성을 열어두는 문형**으로 쓴다("~을 함께 만들어 볼 수 있을 것 같아요", 대표 문형 08-06) — 확정 약속처럼 쓰지 마라. 근거 없는 유형을 넣지 마라. 유도 근거조차 없으면 null.
...
⛔출력 직전 자기검열 — 두 번 훑어라.
[1차: 상투어] descriptions 각각에 다음이 한 단어라도 있으면 그 문장을 구체 사실로 다시 써라: 믿습니다 / 응원합니다·응원해요·지지합니다 / 빛나는 / 든든한 / 소중한 / 특별한 / 함께 성장 / 꿈(을 향해·을 펼치다) / 첫걸음·시작점 / 긍정적인 에너지 / 새로운 내일·미래. 특히 각 후보의 마지막 문장이 "~응원합니다 / ~바랍니다"류 추상 격려로 끝나면 안 된다. 이건 warmth를 죽이라는 게 아니라, 구체에 뿌리내리지 않은 빈 감정어만 걷어내라는 것이다(담백한 온기는 사실 속에서 배어난다).
[2차: 페이오프 — ⚠️descriptions에만 적용] ⭐**descriptions의 마지막 문장만 따로 모아서 읽어라.** 전부 사실 진술로 끝나 있으면 그 글은 얇은 것이다 — 절반 이상은 위 '마무리 규칙 [3] 의미로 닫기'로 고쳐 써라. ⛔단 전부를 그렇게 닫지는 마라(리듬이 죽는다).
[2.5차: 시제] 🚨⭐**완결형으로 닫힌 문장을 전부 잡아내라** — "만듭니다 / 완성했습니다 / 구축했습니다 / 마련했습니다 / 이루어 냈습니다 / 정착시켰습니다"로 끝나는 문장을 찾고, **그 일이 지금도 이어지는 것이면** "만들어 가고 있습니다 / 완성해 가고 있습니다 / 넓혀 가고 있습니다"로 바꿔라. 정말 끝난 사건(지난 콜라보·일회성 행사·수상)만 과거형으로 남긴다. descriptions·활동·콜라보 desc 전부에 적용한다.
[3차: 길이 — activityHints·collabHints] ⭐**각 desc가 세 문장을 넘으면 자른다.** activityHints는 한두 문장, collabHints는 1~2문장이 기준이다. 여기에 페이오프 문장을 붙이지 마라 — 이 필드는 사실 기술이다(위 desc 규칙 참조).`;
...
class NaverGeminiProvider implements SearchProvider {
  ...
  private async gather(query: string, region?: string): Promise<string> {
    ...
    if (web.length) {
      // ⚠️ 라벨이 용도를 좁히면 모델이 그 용도로만 쓴다 — 07-29 실측: 여기에 언론 소개 링크가
      //    멀쩡히 들어와 있는데도(레이지오터 genie.co.kr) press의 url이 안 채워졌다.
      //    "홈페이지·SNS 단서"라고만 써 두니 모델이 여길 '홈페이지 찾는 칸'으로만 봤다.
      //    ※ 네이버 `news` API는 대안이 아니다 — relevance가 아니라 최신순이라 소상공인엔 무관 기사만 온다(실측 기각).
      parts.push("\n[네이버 웹문서 — 홈페이지·SNS·언론 보도 단서 (⭐press url의 유일한 출처)]");
      for (const it of web) {
...
⭐SNS·홈페이지는 이름만으로는 안 나온다 — 전용 검색어로도 반드시 따로 찾아봐: "${query} 인스타", "instagram.com ${query}", "${query} 공식", "${query} 홈페이지".
⭐공식 홈페이지·인스타를 찾았으면 **첫 화면만 보지 마라** — 소개/about·협업·프로젝트·이력·공지·블로그 같은 하위 페이지까지 확인해라. 협업 이력과 활동 기록은 대부분 첫 화면이 아니라 거기에만 적혀 있다. 계정·사이트를 찾으면 그 안의 소개·공지 내용도 사실 정리에 활용해.`
        : round === 3
    ...
    // ⭐조사 프롬프트 정본(대표 조립 07-27) — "브랜드를 소개하는 게 아니라 Brand DB를 만든다"는 관점.
    //   대표 보강 + 레드팀 수정 5곳: ①키워드는 DNA 기준이 아니라 '구별되는 구체어'(DNA는 칩을 읽지 않는다 —
    //   칩의 소비자는 사장님 선택 UI와 소개서다. DNA 기준으로 쓰면 Pool 평탄화가 원천에서 재발)
    //   ②[키워드]는 '한 번만' 규칙의 예외(증류라 반복이 설계) ③"나열 금지"→"가공 없이 옮겨 붙이지 마라"
    //   (출력 자체가 개조식이라 파서와 충돌) ④소상공인 humble-source 허용 복원(thin→재검색 2콜 방지)
    //   ⑤밀집 포맷 유지(모델이 입력 포맷을 미러링해 출력을 조각내지 않게).
    const promptFor = (round: number) => `절대 추측하거나 사실이 아닌 정보를 지어내지 마. 웹에서 확인된 것만 적는다.

【조사의 목적】
이 조사는 브랜드를 최대한 많이 설명하는 것이 아니라, Brand DB를 구축하기 위한 기초 데이터 수집이다.
방문객용 실용 정보(주차·영업시간·가격)보다 브랜드의 정체성·철학·차별점·활동·운영방식을 우선한다.
정확성과 재사용성을 가장 중요하게 생각한다. 브랜드를 평가하지 않고, 과장하지 않는다.
⛔헤지 표현 금지 — "추정된다 / ~인 것으로 보인다 / ~인 듯하다 / 아마 / 가능성이 있다 / 일반적으로"를 쓰지 마라.
확인되지 않았다면 그 자리에 반드시 "확인 안 됨"만 적는다.

"${query}" 브랜드/업체를 웹에서 조사해줘.${searchStrategy(round)}${loc}${ownerBlock}${anchorBlock}

【작성 전 이해 순서】
아래 순서로 브랜드를 이해한 뒤 항목을 작성한다:
①이 브랜드는 무엇을 하는가 ②왜 시작했는가 ③무엇이 다른가 ④무엇을 만들고 있는가 ⑤누구와 연결되고 있는가.
검색 결과 문장을 가공 없이 옮겨 붙이지 마라 — 이해한 뒤 이 브랜드의 사실로 정리해서 적는다.

【작성 규칙】
- 확인된 사실만 작성한다. 하나의 사실은 가장 적절한 항목에 한 번만 적는다(항목 간 반복 금지).
  단 [키워드]는 예외 — 위 내용 전체의 요약·증류라 반복이 정상이다.
- 사실마다 출처를 괄호로 짧게 함께 적는다 — (공식 홈페이지) (네이버 플레이스) (Instagram) (네이버 블로그) (브런치) (유튜브) (지역 기사) (인터뷰) 등 사람이 확인 가능한 형태로. 연도·날짜가 보이면 함께.
- 공식 홈페이지·언론 보도가 있으면 우선하되, 지역 소상공인은 그런 출처가 없는 경우가 많다 — 네이버 플레이스·지도 등록 정보, 블로그·카페 후기, 지역 커뮤니티 글을 폭넓게 활용해라(어디서 봤는지 함께).
- 전체 1800자 이내. 개조식.
- ⭐⭐출력 형식(반드시 지킬 것): 소제목은 아래 목록의 **대괄호 표기 그대로** 한 줄에 쓴다 — [정체] [제품·서비스] …
  ⛔마크다운 볼드(**정체**)·해시(## 정체)·콜론(정체:)으로 바꾸지 마라. 표기가 다르면 뒤 시스템이 읽지 못한다.
- ⭐각 줄은 짧게. '이름 — 짧은 설명' 형태로 쓰고, 이름은 25자 이내로. 설명을 길게 쓰느라 이름이 묻히면 안 된다.
- ⭐대표·직원의 실명은 적지 마라(개인정보). 브랜드명·프로그램명·파트너 기관명은 괜찮다.
- 소제목 설명에 등장하는 단어(워크숍·클래스·펀딩·팔로워 등)를 실제 확인 없이 되풀이해 적지 마라.

[정체] 무엇을 만들고/하는 브랜드인지 한두 줄. 창업 배경·브랜드 철학·시작 계기가 확인되면 2~3줄 추가.
[제품·서비스] 대표 제품·서비스의 이름·종류 (각 한 줄). 업종에 맞게: 요가=하타·빈야사·인요가 / 커피=핸드드립·싱글오리진 / 공방=업사이클링·패치워크 / IT=개인송금·API. ⭐업계에서 통용되는 분류명·유파·기법·방식을 확인했다면 반드시 그대로 적어라 — 이 이름이 이후 브랜드 비교 기준이 된다. ⛔자료에 없으면 절대 생성하지 마라(없는 유파를 붙이면 거짓이 된다).
[특장점] 다른 브랜드와 무엇이 다른가만 — 운영 방식·기술·자격·인증·규모·경력·가격 경쟁력·자체 생산 (예: "직접 로스팅", "8년 차", "은행 대비 낮은 수수료", "15개 언어 상담"). 제품 이름을 여기 반복하지 말고 '차이'만 적어라.
[활동] 브랜드가 스스로 지속적으로 하는 활동만 — 워크숍·클래스·정기 모임·콘텐츠 발행·제품 라인 운영. ⛔상대가 있어야 성립하는 일은 여기 쓰지 말고 [콜라보]로 — 협업·공동제작·입점·팝업 참여·페어 참가·외부 기관 수업. 판별: "누구와 함께"가 붙어야 말이 되면 콜라보다. 그 상대가 없어도 늘 하는 일이어야 활동이다.
[콜라보] 다른 브랜드·기관·공간·작가와 함께한 활동만. 반드시 파트너 이름이 확인된 경우만 작성한다.
  ⭐⭐이 항목은 이 조사에서 가장 값진 정보다 — 파트너 이름이 확인되면 **아끼지 말고 전부** 적어라(최대 8건).
  다른 항목을 줄여서라도 여기를 채운다. 형식은 "파트너명 — 무엇을 함께했는지 20자 내외" 한 줄씩.
[원하는 협업] 브랜드가 직접 밝힌 '함께하고 싶은 파트너·원하는 협업'만 — 인터뷰·모집글·소개글 등 근거 필수. ⛔⛔"받지 않습니다 / 사양합니다 / 정중히 거절합니다"처럼 안 받겠다고 밝힌 문의 유형은 절대 여기 쓰지 마라 — 정반대 뜻이 된다. 그런 문장은 통째로 건너뛴다(예: "유료 홍보·체험단 문의는 받지 않습니다" → 아무것도 적지 않는다).
  ⛔"A를 제외한 다른 협업에는 열려있다" / "A 외의 협업은 환영" 처럼 **거절을 감싸 긍정문으로 바꾸는 것도 금지**다 — 결국 거절 목록이 들어온다.
  ⭐형식은 **짧은 명사구**로("제품 콜라보 희망", "팝업 파트너 모집"). 문장으로 쓰지 마라. 원하는 게 구체적으로 확인 안 되면 "확인 안 됨".
[고객] 주요 고객층 — ⭐사람 집단만(임산부·초보자·지역 주민·직장인·관광객·반려인). ⛔감정·관심사·증상·활동·취미·수업 내용(마음공부·불안·유연성·힐링·뜨개질·요가)은 고객이 아니다 → [제품·서비스]나 [특장점]으로.
  판별: 그 말 뒤에 '~하는 분들'을 붙여야 말이 되면 그건 고객이 아니라 주제다. 확인된 것만.
[알려짐] 언론·방송·매거진·수상 노출 이력만 — 반드시 매체명·프로그램명·수상명 중 하나 이상이 확인된 경우만 (예: "○○매거진 2023년 소개"). 확인 안 되면 그 줄은 생략(막연히 "매체 노출"이라고 쓰지 마라).
  ⭐기사·게시물 **URL이 검색 결과에 실제로 보이면 매체명 옆에 원문 그대로** 함께 적어라 — 소개서의 "소개된 곳들" 블록이 이 링크를 쓴다. ⛔안 보이면 생략(URL 창작은 절대 금지).
[공간] 운영하는 매장·공방·쇼룸·작업실·카페 등과 위치.
[이용/편의시설] 브랜드 성격을 설명하는 시설만 — 요가매트 대여·예약제·단체석·반려동물 동반·포장·클래스 공간. ⛔제외: 화장실·와이파이·정수기·영업시간·전화번호·가격.
[신뢰정보] 공식 홈페이지 URL · 주소.
[키워드] 이 브랜드를 나타내는 키워드 8~15개, 쉼표 구분 (각 2~15자 명사구, 문장 금지).
  ⭐기준은 '이 브랜드에서만 참인 말'이다. 두 갈래를 **같은 급으로** 담아라:
  ①구체어 — 제품명·프로그램명·소재·기법·방식(예: 닥나무가죽, 죠각워크숍, 싱글오리진)
  ②이 공간만의 분위기·정서 표현 — 그 브랜드를 떠올리게 하는 말이면 추상적이어도 좋다(예: 다락방, 유럽풍, 햇살, 아날로그, 수달).
     ⚠️분위기가 곧 정체성인 브랜드(카페·바·서점 등)에서는 ②가 ①보다 중요할 수 있다 — 빼지 마라.
  ⛔뒤로 미룰 것: 업종명 자체(카페·공방·요가원)와 어디에나 붙는 운영어(예약제·주차·포장).
  ⛔사람 이름(대표·직원 실명)은 절대 넣지 마라. 다른 브랜드 것도 섞지 마라.
  ⭐위 항목들에서 확인된 내용으로 **최소 8개는 반드시 채운다**(이 목록이 다음 단계의 주재료다).

⭐인스타그램 — 다음 둘 중 하나면 @핸들을 적어(어디서 봤는지 함께): ①instagram.com/○○ 페이지를 확인했다 ②검색 결과·블로그·기사·소개글 본문에 그 핸들이 실제 문자로 적혀 있었다. ⛔브랜드명·도메인에서 유추해 만들어내는 건 절대 금지. 둘 다 아니면 "인스타: 확인 안 됨".
⭐인스타 개별 게시물 — 이 브랜드 본인 계정의 게시물 URL(instagram.com/p/… 또는 …/reel/…)이 보이면 원문 그대로 나열(최대 5개, 줄마다 하나). 사장님이 직접 쓴 글이라 소개서 재료로 가장 값지다. 다른 계정 게시물 제외.

확실하지 않으면 적지 않는다. 과장하지 않는다. 추측하지 않는다.
브랜드를 소개하는 것이 아니라, Brand DB를 구축한다는 관점으로 작성한다.`;
  ...
  // 보도기사 본문 발췌 → 프롬프트 블록. blockHints(press)의 desc를 범용문장 대신 구체 사실로.
  private pressBlock(d?: string): string {
    return d?.trim()
      ? `[보도기사 본문 발췌]\n${d.trim()}\n\n⭐기사 발췌가 있으면 blockHints(press)의 item desc를 "이색 공간으로 소개됐어요" 같은 범용문장 대신 기사 속 구체 사실(대표 메뉴·이벤트명·인용)로 써라. descriptions의 사실 재료로도 활용해라. 단 ⛔영업시간·가격·기간 한정 정보는 쓰지 마라.\n\n`
      : "";
  }
...
자리가 부족한 뒤 순위 별표는 descriptions(장문)에서 비중 있게 다뤄.\n\n`
    ...
    const verb = verbatim.length
      ...
      : "";
    const prompt = `브랜드명: "${input.name}"\n\n${note}${kw}${star}${verb}${this.igBlock(input.instagramDigest)}${this.digestBlock(input.homepageDigest)}${this.pressBlock(input.pressDigest)}[조사 자료 — 네이버 검색 + 제미나이 웹 조사]\n${input.research}\n\n위 정보로 한 줄 소개 5개, 브랜드 소개 5개(각 3~5문장, 어미는 시스템 지침의 '어미 리듬' — 선언=~합니다/부연=~이에요 혼합), 브랜드 결 단어 2~4개, identity(지역·주소·인스타·홈피)를 뽑아줘.
...
/** 조각화에서 보호할 구분자 → 제어문자. 키 집합은 분리 정규식 `/[,，;·、]/`와 동기. */
const SEP_GUARD: Record<string, string> = {
  ",": "\u0000",
  "\uFF0C": "\u0001",
  ";": "\u0002",
  "\u00B7": "\u0003",
...
const SEP_UNGUARD: Record<string, string> = Object.fromEntries(
  Object.entries(SEP_GUARD).map(([k, v]) => [v, k])
);
function guardParenCommas(s: string): string {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    // ⚠️07-27 실장애: 콤마만 막고 `·`·`;`·`、`는 안 막아서, 아래 분리 정규식이 **괄호 안 가운뎃점**에서
    //    쪼갰다 → "식물성 가죽(선인장" 처럼 여는 괄호만 달린 칩이 나왔다(대표 실측).
    //    분리 정규식 `/[,，;·、]/`의 문자 집합과 SEP_GUARD의 키는 **반드시 같이 유지**할 것.
    const isDigitComma =
      (ch === "," || ch === "\uFF0C") && /\d/.test(arr[i - 1] ?? "") && /\d/.test(arr[i + 1] ?? "");
    const guarded = SEP_GUARD[ch];
    if (guarded && (depth > 0 || isDigitComma)) {
      out += guarded;
    } else {
      out += ch;
    }
...
export function extractChipsFromResearch(
  ...
  // 🚨소제목 표기 흔들림 방어(07-27 실장애) — 모델이 [정체] 대신 **정체** / ## 정체 / 정체: 로 쓰면
  //   아래 split이 섹션을 하나도 못 잡아 **칩이 통째로 증발한다**. 실제로 2.5-flash 1라운드가
  //   마크다운 볼드로 써서 칩 3개(그 메모엔 콜라보가 4건 들어 있었다) → 재검색 2콜 낭비.
  //   프롬프트로 형식을 요구하되(요청), 파서도 알려진 소제목명이면 대괄호로 정규화한다(보장).
  const normalized = body.replace(
    /^[\s#>*]*\**\s*([가-힣A-Za-z\u00B7/ ]{2,12}?)\s*\**\s*:?\s*$/gm,
    (m, raw: string) => (CHIP_SECTIONS[raw.trim()] ? `[${raw.trim()}]` : m)
  );
  ...
  const push = (raw: string, sec: { label: string; factual: boolean }) => {
    let text = raw
      ...
      .trim();
    // ⭐칩에 따옴표는 쓰지 않는다(대표 지시 07-27) — 짝이 맞든 안 맞든 벗기고 내용만 남긴다.
    //   짝 맞는 인용도 칩에선 의미가 없고(프로그램명은 그 자체로 읽힌다), 짝이 깨진 건 잘림 신호다.
    //   (구 규칙: 홀수 개일 때만 제거 — 한담누리 07-21)
    text = text.replace(/["'“”‘’]/g, "").replace(/\s+/g, " ").trim();
    // ⭐목록 번호 접두 제거(대표 지시 07-27) — 조사 메모의 "1. 산전요가 개인레슨"이 마침표만 떨어져
    //   "1 산전요가 개인레슨"으로 남던 것. 숫자가 곧 의미인 칩("8년 차"·"1:1 수업")은 뒤에
    //   공백+글자가 오지 않으므로 건드리지 않는다.
    text = text.replace(/^\d+\s*[.)]\s*/, "").replace(/^\d+\s+(?=[가-힣A-Za-z])/, "").trim();
    // ⭐괄호는 쌍으로만(대표 지시 07-27) — 조각화로 한쪽이 잘리면 "식물성 가죽(선인장"이 남는다.
    //   여는 괄호만 있으면 그 뒤를 버리고, 고아 닫는 괄호는 지운다.
    ...
    // 🚨거절·수용 서술은 칩이 아니다(대표 prod QA 07-27) — [원하는 협업]에서 반복 유입.
    //   "유료 홍보/체험단/대행사 관련 문의는 받지 않음" · "그 외 다른 협업에는 열려있음"
    //   프롬프트에 "거절 문장 금지"를 넣었는데도 모델이 **"~를 제외한 다른 협업에는 열려있다"**로
    //   감싸 우회했다 → 프롬프트는 요청, 차단은 파서가.
    //   ⚠️'없음'은 여기 넣지 말 것 — "웨이팅 없음"은 오히려 장점 칩이다(기존 정책).
    if (/받지\s*않|않음$|사양(합니다|해)|정중히\s*거절|제외한|외의|열려\s*있/.test(text)) return false;
...
/** 기계적 교정(대표 지시 2026-07-20) — 규칙이 명백해 오교정 위험이 없는 것만 손댄다.
 *  띄어쓰기 전반은 모델 자기검열(BRAND_VOICE 맞춤법 조항)에 맡기고, 여기선 기계가 확실히
 *  판단 가능한 공백·문장부호 정리만. ⚠️의미 판단이 필요한 교정(되/돼 등)은 넣지 않는다. */
export function tidyTypography(text: string): string {
  if (!text) return text;
  return text
    .replace(/[ \t]{2,}/g, " ") // 중복 공백 → 한 칸
    .replace(/\s+([.,!?%])/g, "$1") // 문장부호 앞 공백 제거
    .replace(/([([{])\s+/g, "$1") // 여는 괄호 뒤 공백
    .replace(/\s+([)\]}])/g, "$1") // 닫는 괄호 앞 공백
    // 문장부호 뒤 공백 — ⚠️앞이 '한글'일 때만. 안 그러면 canvasgarden.shop → "canvasgarden. shop"으로 깨진다.
    .replace(/([가-힣][.!?])([가-힣A-Za-z])/g, "$1 $2")
    .replace(/ +\n/g, "\n") // 줄 끝 공백
    .trim();
}
