...
// ── 소셜 로그인 온보딩(/welcome) ─────────────────────────────────────────────
// 구글은 이름·이메일만 준다. 우리 계정은 **휴대폰번호가 필수**다(🔁09-17 브랜드명은 선택으로 — 하루 팝업으로 브랜드 없는 사람도 온다). 브랜드명은
...
// ⭐여기서 막히는 사람 상당수는 **같은 사장님이 다른 로그인 방법으로 들어온 것**이다(08-15 설계).
//   소셜 이메일이 기존 계정과 다르면 Supabase의 자동 연결이 못 묶어서 새 계정으로 오는데,
//   그때 부딪히는 첫 관문이 이 중복 검사다. **막고 끝내지 말고 돌아갈 길을 알려준다** —
//   원래 방법으로 로그인한 뒤 `/my`의 「연결된 로그인」에서 붙이면 다음부터 이 방법으로 들어온다.
//   (그 화면이 실제로 있어야 이 안내가 거짓말이 안 된다 — `my/LinkedAccounts.tsx`.)
const LINK_HINT =
  " 원래 쓰시던 방법으로 로그인한 뒤 [내 정보 → 연결된 로그인]에서 연결하시면, 다음부터 이 방법으로 들어오실 수 있어요.";
const ONBOARD_DUP = {
