 ...
 *    이 메일은 결제 직후에 나가서 아직 수락 전이다. 그래서 표를 먼저 말하고 창은 뒤에 붙인다. */
const CANCEL_POLICY_LINE =
  "이용일 7일 전까지 전액, 3일 전까지 70%, 1일 전까지 50%를 돌려드리고 당일은 환불이 없어요. 사장님이 수락하신 뒤 한 시간 안에 취소하시면 날짜와 상관없이 전액이에요.";
...
export function buildBookingCancelledToGuest(
  ...
  const lead = refund === 0
    // 0원은 당일 취소뿐이다(`guestCancelRefundRate` — 1일 전까지는 50%라도 돌아간다).
