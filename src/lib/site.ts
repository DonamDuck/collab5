// 사이트의 기준 주소 한 곳 — 사이트맵·robots·링크 미리보기가 전부 이걸 쓴다.
// ⚠️ 상수로 박는다(환경변수 아님): 프리뷰 배포마다 주소가 달라지면 사이트맵이 vercel.app 주소를
//    가리키고, 구글은 그걸 **다른 사이트의 목록**으로 보고 통째로 무시한다.
export const SITE_URL = "https://collab5.co.kr";

/** 링크 미리보기(카톡·슬랙·X) 기본 썸네일 — 1200×630. **사진이 없는 페이지는 전부 이걸로 떨어진다.**
 *
 *  ⭐`?v=` 를 붙인 채로 둔다. 카톡·페북 스크래퍼는 이미지를 **URL 단위로** 캐시해서, 파일 내용만
 *    바꾸고 주소를 그대로 두면 **옛 썸네일이 계속 나간다**(08-16 로고 교체 때 실제로 겪음).
 *    그림을 갈 때마다 v를 올릴 것.
 *
 *  🪤 카톡은 이미지뿐 아니라 **페이지 스크랩 결과도** 따로 캐시한다. v를 올려도 이미 퍼진 링크는
 *     그대로일 수 있다 — 급하면 카카오 개발자 도구에서 주소별로 초기화해야 한다
 *     (https://developers.kakao.com/tool/debugger — 「카카오톡 URL 메타정보 관리」).
 *     🚨09-04에 대표가 눌렀더니 404였다. 옛 주소 `/tool/clear/og`는 카카오가 없앴다.
 *     ⭐**남의 서비스 주소는 조용히 죽는다** — 문서에 박아두고 몇 달 뒤 그대로 안내하면
 *     받는 사람이 막힌다. 안내하기 «전»에 한 번 열어 볼 것.
 */
export const OG_IMAGE = "/og-image.png?v=4";  // v4 = 홈 헤더와 «같은» 워드마크로 교체(09-04). v3 = 로고 초록을 키위(#98FF5C)로 교정(08-16)

// ─── 고객센터 연락 창구 (2026-09-16) ───
// ⭐한 벌로 둔다 — 푸터·고객센터·홈 플로팅·하루 팝업 화면이 같은 값을 본다. 한 곳만 바꿔도 전부 따라온다.
/** 카카오톡 채널 홈(대표 09-16). 09-23 기준 아무 데서도 안 쓴다. 상담은 전부 `KAKAO_CHAT_URL`이다.
 *  「채널 추가」 같은 홈 링크를 붙일 때 쓰려고 대표가 준 값 그대로 남겨 둔다. */
export const KAKAO_CHANNEL_URL = "https://pf.kakao.com/_xlMeiX";
/** 누르면 바로 1:1 채팅이 열리는 주소. 상담 버튼은 이쪽을 쓴다 — 채널 홈에서 한 번 더 누르게 하지 않는다.
 *  ⚠️09-16에 둘 다 열어 확인했다(200). 남의 서비스 주소는 조용히 죽으니 바꿀 땐 다시 열어 볼 것. */
export const KAKAO_CHAT_URL = "https://pf.kakao.com/_xlMeiX/chat";
export const SUPPORT_EMAIL = "dudejrthd@gmail.com";
export const SUPPORT_PHONE = "010-2060-1629";
