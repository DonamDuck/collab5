...
/** 🏠등록증 주소 → 주소 찾기(카카오 우편번호) 표기 (09-20 대표 「너 추천대로 고고」).
 *  등록증은 「서울특별시 성동구…」처럼 긴 시도 이름을 쓰고, 주소 찾기는 「서울 성동구…」다. 「등록증 주소로 채우기」가
 *  긴 이름을 그대로 넣으면 같은 동네가 두 표기로 저장된다(동네 칸 `area`는 주소의 앞 두 조각이라 「서울특별시 성동구」가 따로 선다).
 *  📏표는 «09-20 주소 찾기 실측»이다. 도청·시청 주소를 등록 폼의 주소 찾기로 골라 주소 칸에 들어간 글자를 그대로 옮겼다.
 *    ⚠️줄이지 «않는» 시도가 넷이다. 세종특별자치시·제주특별자치도·강원특별자치도·전북특별자치도는 주소 찾기도 긴 이름을 쓴다.
 *    ⚠️옛 이름은 주소 찾기의 지금 이름으로 바꾼다. 강원도→강원특별자치도, 전라북도→전북특별자치도,
 *      광주광역시·전라남도→전남광주통합특별시(주소 찾기가 두 곳을 이 이름 하나로 돌려준다. 구·시·군 이름은 그대로다).
 *  맨 앞 한 조각만 본다. 이미 짧은 표기(「서울 …」)와 뒤쪽 글자(괄호 속 동네 이름 등)는 손대지 않는다. */
const POSTCODE_SIDO: Record<string, string> = {
  서울특별시: "서울", 부산광역시: "부산", 대구광역시: "대구", 인천광역시: "인천",
  대전광역시: "대전", 울산광역시: "울산", 경기도: "경기",
  충청북도: "충북", 충청남도: "충남", 경상북도: "경북", 경상남도: "경남",
  강원도: "강원특별자치도", 전라북도: "전북특별자치도",
  광주광역시: "전남광주통합특별시", 전라남도: "전남광주통합특별시",
};
export function postcodeSido(addr: string): string {
  const a = (addr ?? "").trim();
  const m = /^(\S+)(\s[\s\S]*)?$/.exec(a);
  if (!m) return a;
