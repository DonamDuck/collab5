...
export const MOCK_MAIL_KINDS: { kind: string; label: string }[] = [
  ...
  { kind: "admin-daily-trouble", label: "아침 요약 → 대표 슬랙 · 예약을 못 읽고 리마인드도 멈춘 날" },
  // 💸09-19 오후 대표 — 거래 알림은 슬랙에만 가요(슬랙 주소가 없으면 안 가요). 사람은 회원 번호로만.
  { kind: "admin-payment", label: "거래 → 대표 슬랙 · 결제 완료 (예약 번호·주문번호·금액·회원 번호)" },
  { kind: "admin-cancel", label: "거래 → 대표 슬랙 · 손님 취소, 일부 환불" },
  { kind: "admin-cancel-full", label: "거래 → 대표 슬랙 · 손님 취소, 전액 환불 (수락 전)" },
  { kind: "admin-cancel-sameday", label: "거래 → 대표 슬랙 · 손님 취소, 당일이라 환불 없음" },
